...
const FEATURE_PILLS = [
  ...
  { key: "featureScenarios", fallback: "38 scenarios" },
  { key: "featureModes", fallback: "5 coaching modes" },
...
export function HomeView() {
  ...
  const STEPS = [
    { n: 1, title: tr("step1Title"), desc: tr("step1Desc") },
    { n: 2, title: tr("step2Title"), desc: tr("step2Desc") },
    { n: 3, title: tr("step3Title"), desc: tr("step3Desc") },
  ];
  ...
  return (
    <div className="relative min-h-[100dvh] overflow-x-hidden bg-ivory text-ink">
      ...
      <section className="relative z-10 mx-auto max-w-6xl px-5 pb-20 sm:px-8 lg:pb-28">
        <h2 className="font-serif text-3xl font-semibold tracking-tight text-ink sm:text-4xl">{tr("howItWorksTitle")}</h2>
        <div className="mt-8 grid gap-10 lg:grid-cols-[1fr_.9fr] lg:items-center lg:gap-16">
          {/* Steps */}
          <ol className="space-y-6">
            {STEPS.map((s) => (
              <li key={s.n} className="flex gap-4">
                <span className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-2xl bg-coral-soft font-mono text-sm font-bold text-coral-dark">{s.n}</span>
                <div>
                  <p className="font-semibold text-ink">{s.title}</p>
                  <p className="mt-1 max-w-md text-[15px] leading-relaxed text-ink-soft">{s.desc}</p>
                </div>
              </li>
            ))}
          </ol>
          ...
          <div>
            <p className="mb-2 font-mono text-[11px] font-semibold uppercase tracking-[0.14em] text-clay">{tr("reportPreviewCaption")}</p>
            <div className="rounded-3xl border border-line bg-cream p-5 shadow-xl">
              <div className="flex items-center justify-between border-b border-line pb-3">
                <span className="font-serif text-lg font-semibold text-ink">{tr("feedbackTitle")}</span>
                <span className="flex flex-col items-end gap-0.5">
                  <span className="font-mono text-[10px] uppercase tracking-wide text-clay">{tr("estimatedLevel")}</span>
                  <span className="rounded-lg bg-forest-soft px-2.5 py-0.5 font-mono text-sm font-bold text-forest">B2</span>
                </span>
              </div>
              <div className="mt-4">
                ...
                <ul className="mt-1.5 space-y-1 text-[13px] leading-snug text-ink-soft">
                  <li>Confident answers with good control of past tenses.</li>
                  <li>Natural small talk and polite question forms.</li>
                </ul>
              </div>
              <div className="mt-4">
                <p className="flex items-center gap-1.5 text-xs font-semibold text-coral-dark">
                  <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M13 7l5 5-5 5M6 12h12" /></svg>
                  {tr("improvements")}
                </p>
                <ul className="mt-1.5 space-y-1 text-[13px] leading-snug text-ink-soft">
                  <li>“I have visited” → “I visited” for a finished past time.</li>
                  <li>Link ideas with <span className="text-ink">however, so, because</span>.</li>
                </ul>
              </div>
            </div>
          </div>
